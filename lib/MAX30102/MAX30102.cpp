...
bool MAX30102::begin()
{
    Wire.begin(myData.sda_pin, myData.scl_pin); //sda D1, scl D2
    ...
    setMode(DEFAULT_MODE); // 2
    setLedsPulseWidth(DEFAULT_PULSE_WIDTH); // 3
    setSamplingRate(DEFAULT_SAMPLING_RATE); // 1 -> 100hz
    setLedsCurrent(DEFAULT_IR_LED_CURRENT, DEFAULT_RED_LED_CURRENT); // 123, 39
	setAdcRange(DEFAULT_ADC_RANGE); // 2
	setSamplingAverage(DEFAULT_SAMPLING_AVERAGE); // 0
	setFIFORolloverEnable(true); // 1
    return true;
}
